...
Texture2D<float4> albedo_tex : register(t0); // + alpha
Texture2D<float3> roughness_tex : register(t1); 
Texture2D<float3> metalness_tex : register(t2);
Texture2D<float3> normal_tex : register(t3);
...
float4 main(PS_INPUT input) : SV_TARGET
{
    ...
    float3 view_dir = normalize(camera - input.Position.xyz);
